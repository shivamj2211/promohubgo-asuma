...
export default function SocialChannelsPage() {
  ...
  const updateSocial = (key: PlatformKey, field: keyof SocialData, value: string) => {
    setSocials((prev) => {
      const current = prev[key]
      const next: SocialData = { ...current, [field]: value }

      // auto URL when username changes
      if (field === 'username') {
  ...
  // ✅ Convert local socials state → backend payload
  const buildPayload = () => {
    const payload = (Object.keys(socials) as PlatformKey[])
      .map((platform) => {
  ...
  // ✅ Skip: clear socials in DB (so no partial old data remains)
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white p-4 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
        <div className="max-w-xl mx-auto space-y-2">
          ...
          <button
            ...
            onClick={handleSkip}
            disabled={saving}
            className="w-full text-center text-[11px] text-gray-500 underline underline-offset-2 disabled:opacity-50"
          >
            Skip for now
          </button>
        </div>
