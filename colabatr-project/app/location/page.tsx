'use client'
import { useRouter } from 'next/navigation'
import Link from 'next/link'
import { Button } from '@/components/ui/button'
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
  DialogFooter,
} from '@/components/ui/dialog'
import { useState, useEffect, useRef } from 'react'

type CitySuggestion = { city: string; state: string }

type PincodeOffice = {
  officename: string
  pincode: number
  district: string
  statename: string
  latitude: string
  longitude: string
}

export default function Location() {
  const [location, setLocation] = useState('') // city + state
  const [showDropdown, setShowDropdown] = useState(false)
  const [touched, setTouched] = useState(false)
  const router = useRouter()
  // City suggestions
  const [suggestions, setSuggestions] = useState<CitySuggestion[]>([])
  const [loading, setLoading] = useState(false)

  // Address-related fields
  const [pincode, setPincode] = useState('')
  const [stateName, setStateName] = useState('')
  const [fullAddress, setFullAddress] = useState('') // optional
  const [pinError, setPinError] = useState<string | null>(null)

  // Offices for the current pincode
  const [offices, setOffices] = useState<PincodeOffice[]>([])
  const [selectedOfficeName, setSelectedOfficeName] = useState('')
  const [useManualOffice, setUseManualOffice] = useState(false)
  const [manualOfficeName, setManualOfficeName] = useState('')

  // Dialog states
  const [saveDialogOpen, setSaveDialogOpen] = useState(false)
  const [cityDialogOpen, setCityDialogOpen] = useState(false)

  const debounceRef = useRef<number | null>(null)

  // ---------- City suggestions (for city/state search) ----------
  useEffect(() => {
    if (debounceRef.current) {
      window.clearTimeout(debounceRef.current)
    }

    debounceRef.current = window.setTimeout(() => {
      const q = location.trim()
      if (!q) {
        setSuggestions([])
        return
      }

      setLoading(true)
      fetch(`/api/cities?q=${encodeURIComponent(q)}`)
        .then((res) => res.json())
        .then((data) => {
          setSuggestions(Array.isArray(data) ? data : [])
        })
        .catch(() => setSuggestions([]))
        .finally(() => setLoading(false))
    }, 180)

    return () => {
      if (debounceRef.current) window.clearTimeout(debounceRef.current)
    }
  }, [location])

  // ---------- Pincode → offices + auto-fill ----------
  useEffect(() => {
    const pin = pincode.trim()

    if (!pin) {
      setPinError(null)
      setOffices([])
      setSelectedOfficeName('')
      return
    }

    if (!/^\d{6}$/.test(pin)) {
      setPinError('Please enter a valid 6-digit pincode')
      setOffices([])
      setSelectedOfficeName('')
      return
    }

    setPinError(null)

    const controller = new AbortController()

    fetch(`/api/cities?pincode=${pin}`, { signal: controller.signal })
      .then((res) => res.json())
      .then((data: { offices: PincodeOffice[] }) => {
        if (!data || !Array.isArray(data.offices) || data.offices.length === 0) {
          setPinError('Pincode not found')
          setOffices([])
          setSelectedOfficeName('')
          return
        }

        setOffices(data.offices)

        const primary = data.offices[0]

        // Always update auto fields when pincode changes
        setStateName(primary.statename)
        setLocation(`${primary.district}, ${primary.statename}`)

        if (!useManualOffice) {
          setSelectedOfficeName(primary.officename)
        }

        setFullAddress((prev) =>
          prev.trim()
            ? prev // keep user-typed address if already there
            : `${primary.officename}, ${primary.district}`
        )
      })
      .catch((err) => {
        if (err.name !== 'AbortError') {
          setPinError('Error fetching pincode details')
        }
      })

    return () => controller.abort()
  }, [pincode, useManualOffice])

  const handleLocationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setLocation(e.target.value)
    setShowDropdown(true)
  }

  const handleLocationBlur = () => {
    setTouched(true)
    setTimeout(() => setShowDropdown(false), 200)
  }

  const handleSelectCity = (city: string, state: string) => {
    setLocation(`${city}, ${state}`)
    setShowDropdown(false)
  }

  const handleLocationClear = () => {
    setLocation('')
    setShowDropdown(true)
  }

  const handleContinue = () => {
    if (!canContinue) return
    setSaveDialogOpen(true)
    // Here you can also call your backend API before opening dialog if needed
  }

  const handleReset = () => {
    setLocation('')
    setShowDropdown(false)
    setTouched(false)
    setSuggestions([])

    setPincode('')
    setStateName('')
    setFullAddress('')
    setPinError(null)

    setOffices([])
    setSelectedOfficeName('')
    setUseManualOffice(false)
    setManualOfficeName('')
  }

  const locationError = touched && !location.trim()

  const handleOfficeSelect = (value: string) => {
    setSelectedOfficeName(value)

    // Auto-fill address based on office, but user can still edit
    const office = offices.find((o) => o.officename === value)
    if (office && !fullAddress.trim()) {
      setFullAddress(`${office.officename}, ${office.district}`)
    }
  }

  const toggleManualOffice = () => {
    setUseManualOffice((prev) => !prev)

    if (!useManualOffice) {
      // going to manual
      setSelectedOfficeName('')
    } else {
      // going back to dropdown
      setManualOfficeName('')
    }
  }

  // “I don't see my city” → open dialog
  const handleCityHelp = () => {
    setCityDialogOpen(true)
  }

  // ---------- Validation for Continue ----------
  const areaValue = (useManualOffice ? manualOfficeName : selectedOfficeName).trim()
  const isValidPincode = /^\d{6}$/.test(pincode.trim()) && !pinError
  const canContinue =
    location.trim().length > 0 &&
    isValidPincode &&
    stateName.trim().length > 0 &&
    areaValue.length > 0
  // fullAddress is optional

  return (
    <>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50">
        <div className="w-full max-w-xl"> {/* increased width */}
          {/* Logo */}
          <div className="text-center mb-8">
            <Link href="/">
              <div className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                promoHubgo
              </div>
            </Link>
          </div>

          {/* Card */}
          <div className="bg-white rounded-xl shadow-lg p-8 md:p-10">
            {/* Progress */}
            <div className="mb-8">
              <div className="h-1 bg-indigo-600 rounded-full" style={{ width: '95%' }} />
            </div>

            <h1 className="text-3xl font-bold text-gray-900 mb-2">Where are you located?</h1>
            <p className="text-gray-600 mb-6 text-sm">
              This helps us show your profile to the right audience
            </p>

            <div className="space-y-4 mb-8">
              {/* 1️⃣ Pincode first */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Pincode
                </label>
                <input
                  type="text"
                  value={pincode}
                  onChange={(e) =>
                    setPincode(e.target.value.replace(/\D/g, '').slice(0, 6))
                  }
                  placeholder="e.g. 504296"
                  className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-600"
                />
                {pinError && (
                  <p className="mt-1 text-xs text-red-500">{pinError}</p>
                )}
              </div>

              {/* 2️⃣ City / Location search */}
              <div className="relative">
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  City / Location
                </label>
                <input
                  type="text"
                  value={location}
                  onChange={handleLocationChange}
                  onFocus={() => setShowDropdown(true)}
                  onBlur={handleLocationBlur}
                  placeholder="Search city or state..."
                  className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 pr-10 transition ${
                    locationError
                      ? 'border-red-500 focus:ring-red-600'
                      : 'border-gray-300 focus:ring-indigo-600'
                  }`}
                />
                {location && (
                  <button
                    type="button"
                    onClick={handleLocationClear}
                    className="absolute right-3 top-[34px] text-gray-400 hover:text-gray-600"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M6 18L18 6M6 6l12 12"
                      />
                    </svg>
                  </button>
                )}

                {/* City dropdown */}
                {showDropdown && suggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-300 rounded-xl shadow-lg z-10 max-h-64 overflow-y-auto">
                    {loading && (
                      <div className="p-4 text-sm text-gray-500">Searching...</div>
                    )}
                    {suggestions.map((item, index) => (
                      <button
                        key={index}
                        type="button"
                        onClick={() => handleSelectCity(item.city, item.state)}
                        className="w-full text-left px-4 py-3 hover:bg-indigo-50 border-b border-gray-200 last:border-b-0 transition"
                      >
                        <div className="font-medium text-gray-900">{item.city}</div>
                        <div className="text-sm text-gray-500">{item.state}</div>
                      </button>
                    ))}
                  </div>
                )}

                {/* No city results */}
                {showDropdown && !loading && suggestions.length === 0 && location.trim() && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-300 rounded-xl shadow-lg z-10 p-4">
                    <p className="text-gray-500 text-center">No cities found</p>
                  </div>
                )}
              </div>

              {locationError && (
                <p className="text-red-500 text-sm">Location is required</p>
              )}

              {/* 3️⃣ Row: State + Area / Locality */}
              <div className="grid gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">
                    State
                  </label>
                  <input
                    type="text"
                    value={stateName}
                    onChange={(e) => setStateName(e.target.value)}
                    placeholder="State"
                    className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-600"
                  />
                </div>

                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-xs font-medium text-gray-600">
                      Area / Locality
                    </label>
                    <button
                      type="button"
                      onClick={toggleManualOffice}
                      className="text-[11px] text-indigo-600 hover:underline"
                    >
                      {useManualOffice
                        ? 'Use suggestions from pincode'
                        : "I don't see my area, enter manually"}
                    </button>
                  </div>

                  {/* Dropdown mode */}
                  {!useManualOffice && (
                    <select
                      value={selectedOfficeName}
                      onChange={(e) => handleOfficeSelect(e.target.value)}
                      disabled={offices.length === 0}
                      className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-600 bg-white"
                    >
                      {offices.length === 0 ? (
                        <option value="">No areas loaded yet</option>
                      ) : (
                        <>
                          <option value="">Select area / locality</option>
                          {offices.map((office, idx) => (
                            <option key={`${office.officename}-${idx}`} value={office.officename}>
                              {office.officename} ({office.district})
                            </option>
                          ))}
                        </>
                      )}
                    </select>
                  )}

                  {/* Manual mode */}
                  {useManualOffice && (
                    <input
                      type="text"
                      value={manualOfficeName}
                      onChange={(e) => setManualOfficeName(e.target.value)}
                      placeholder="Type your area / locality"
                      className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-600"
                    />
                  )}
                </div>
              </div>

              {/* 4️⃣ Full Address (optional) */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Full address <span className="text-[11px] text-gray-400">(optional)</span>
                </label>
                <textarea
                  value={fullAddress}
                  onChange={(e) => setFullAddress(e.target.value)}
                  rows={3}
                  placeholder="House / flat, street, landmark... (optional)"
                  className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-600 resize-none"
                />
              </div>
            </div>

            {/* Actions: Continue + Reset */}
            <div className="flex flex-col gap-3">
              <Button
                onClick={handleContinue}
                disabled={!canContinue}
                className="w-full py-3 bg-gray-900 text-white font-semibold rounded-full hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
                Continue
              </Button>

              <Button
                type="button"
                variant="outline"
                onClick={handleReset}
                className="w-full py-3 rounded-full"
              >
                Reset
              </Button>
            </div>

            {/* I don't see my city */}
            <p
              className="text-center text-gray-600 text-sm mt-6 cursor-pointer hover:text-indigo-600"
              onClick={handleCityHelp}
            >
              I don&apos;t see my city
            </p>
          </div>
        </div>
      </div>

      {/* ✅ Dialog: Location saved */}
      <Dialog open={saveDialogOpen} onOpenChange={setSaveDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Location saved</DialogTitle>
            <DialogDescription>
              We&apos;ve captured your location details. You can update them later from your
              profile if needed.
            </DialogDescription>
          </DialogHeader>

          <div className="mt-4 space-y-2 text-sm text-gray-700">
            <p>
              <span className="font-medium">City / Location:</span> {location}
            </p>
            <p>
              <span className="font-medium">Pincode:</span> {pincode}
            </p>
            <p>
              <span className="font-medium">State:</span> {stateName}
            </p>
            <p>
              <span className="font-medium">Area / Locality:</span>{' '}
              {(useManualOffice ? manualOfficeName : selectedOfficeName) || '(not specified)'}
            </p>
            <p>
              <span className="font-medium">Full address:</span>{' '}
              {fullAddress || '(optional, not provided)'}
            </p>
          </div>

          <DialogFooter className="mt-6">
                    <Button
          onClick={() => {
            setSaveDialogOpen(false)
            router.push('/profile')   // 👈 redirect to new page
          }}
          className="w-full sm:w-auto"
        >
          Okay, continue
        </Button>

          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* ✅ Dialog: I don't see my city / outside India */}
      <Dialog open={cityDialogOpen} onOpenChange={setCityDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>City not found?</DialogTitle>
            <DialogDescription>
              No worries — our team is here to help you.
            </DialogDescription>
          </DialogHeader>

          <div className="mt-4 space-y-3 text-sm text-gray-700">
            <p>
              It looks like your city isn&apos;t appearing in our list. This can happen if:
            </p>
            <ul className="list-disc list-inside space-y-1">
              <li>You are trying to register from outside India, or</li>
              <li>Your city is not yet added to our service coverage.</li>
            </ul>
            <p className="mt-2">
              <span className="font-medium">promoHubgo currently serves locations in India only.</span>{' '}
              If you&apos;re from another country, we&apos;ll notify you as soon as we launch in your
              region.
            </p>
            <p className="mt-2">
              Please reach out to our customer care team from the{' '}
              <span className="font-medium">Help / Support</span> section in the app for manual
              assistance.
            </p>
          </div>

          <DialogFooter className="mt-6">
            <Button onClick={() => setCityDialogOpen(false)} className="w-full sm:w-auto">
              Got it
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  )
}
